...
struct wezel* szukaj(struct wezel *start, int wartosc)  
 ...
 struct wezel* poprzednik(struct wezel *start, int wartosc) 
 {
	 struct wezel* =szukaj(start,wartosc);
...
int main(int argc, char *argv[])
 ...
 dodawanie(1,root);
  ...
  printf("Wartość max drzewa: ");
  struct wezel *max=naj_prawo(root);
  printf("%d \n",max->wartosc);
